...
let make = (~message, ~currentState, ~newState, ~isCreator, _) => {
    ...
    {isCreator ? <img className="editIcon" src="https://image.flaticon.com/icons/png/512/61/61848.png" onClick={_ => DatabaseFunctions.deleteMessage(message["id"], currentState, newState)} /> : React.null}
    {isCreator ? <img className="editIcon" src="https://simpleicon.com/wp-content/uploads/pencil-256x256.png" onClick={(_) => setIsOpen((_previousState) => true)} />: React.null}
